...
import {
  Dumbbell, Timer, Flame, ChevronRight,
...
import {
  ...
  setStartDate as persistStartDate, markWorkoutComplete,
  isWorkoutCompleted, toDateStr, getTodayNutrition
} from "@/lib/storage";
...
export default function Home() {
  ...
  // Nutrição do dia para o card resumo
  const todayNutrition = getTodayNutrition();
  const totalProtein = todayNutrition.meals.filter(m => m.done).reduce((s, m) => s + m.protein, 0);
  const totalKcal = todayNutrition.meals.filter(m => m.done).reduce((s, m) => s + m.kcal, 0);
  const proteinPct = Math.min(100, Math.round((totalProtein / DAILY_TARGETS.protein) * 100));
  const mealsLogged = todayNutrition.meals.filter(m => m.done).length;
      ...
      <div className="px-4 py-5 space-y-4">
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-1">
                {RPE_COLORS.map((color, i) => {
                  ...
                  return (
                    <div key={i} className={cn("h-2 flex-1 rounded-sm", color, i > targetIdx && "opacity-20")} />
                  );
                })}
              </div>
            </div>
          ...
          {workout.exercises.length > 0 && (
            <div className="space-y-1.5 mb-4">
              ...
              {workout.exercises.length > 5 && (
                <button onClick={() => navigate("/plano")} className="text-xs text-slate-400 hover:text-white transition-colors flex items-center gap-1 mt-1">
        ...
        >
          ...
          <div className="grid grid-cols-7 gap-1.5">
            {weekDays.map(({ date, workout: w, isToday }, i) => {
              ...
              return (
                <div key={i} className={cn(
                  ...
                  <span className={cn("text-sm font-bold mt-0.5", isToday ? "text-blue-300" : "text-white")}>{date.getDate()}</span>
                  <span className="text-[10px] mt-1">{completed && w.type !== "descanso" ? "✅" : WORKOUT_ICONS[w.type]}</span>
                </div>
              );
            })}
          </div>
              ...
              <>
                ...
                {latestWeight.weight < 119 && (
                  <div className="flex items-center gap-1 mt-2 text-emerald-400 text-xs font-semibold">
                    <TrendingDown className="w-3.5 h-3.5" />-{(119 - latestWeight.weight).toFixed(1)} kg
                  </div>
                )}
              </>
            ) : (
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            <div className="flex gap-1 mt-3 flex-wrap">
              {CHECKLIST_ITEMS.map((item) => (
                <div key={item.id} className={cn("w-4 h-4 rounded-sm", checklist[item.id] ? "bg-blue-500" : "bg-slate-700")} />
              ))}
            </div>
          </motion.div>
        </div>

        {/* ── Card de Nutrição (resumo) ── */}
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.22 }}
          className="bg-slate-900 rounded-2xl border border-white/10 p-4 cursor-pointer hover:bg-slate-800/60 transition-colors"
          onClick={() => navigate("/nutricao")}
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Utensils className="w-4 h-4 text-amber-400" />
              <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Alimentação Hoje</p>
            </div>
            <ChevronRight className="w-4 h-4 text-slate-600" />
          </div>
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div>
              <p className={cn("text-xl font-black", totalProtein >= DAILY_TARGETS.protein ? "text-emerald-400" : totalProtein > 0 ? "text-amber-400" : "text-slate-500")}>
                {totalProtein}g
              </p>
              <p className="text-[10px] text-slate-500 uppercase tracking-wide mt-0.5">Proteína</p>
              <p className="text-[10px] text-slate-600">meta: {DAILY_TARGETS.protein}g</p>
            </div>
            <div>
              <p className={cn("text-xl font-black", totalKcal > 0 ? "text-white" : "text-slate-500")}>{totalKcal}</p>
              <p className="text-[10px] text-slate-500 uppercase tracking-wide mt-0.5">Kcal</p>
              <p className="text-[10px] text-slate-600">meta: {DAILY_TARGETS.kcal}</p>
            </div>
            <div>
              <p className={cn("text-xl font-black", mealsLogged > 0 ? "text-white" : "text-slate-500")}>{mealsLogged}/5</p>
              <p className="text-[10px] text-slate-500 uppercase tracking-wide mt-0.5">Refeições</p>
              <p className="text-[10px] text-slate-600">registradas</p>
            </div>
          </div>
          {/* Barra de proteína */}
          <div>
          ...
          {todayNutrition.beliscos && (
            <p className="text-xs text-red-400 mt-2 flex items-center gap-1">⚠️ Belisco registrado hoje</p>
          )}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          ...
          <div className="space-y-1">
            {CHECKLIST_ITEMS.map((item) => (
              <motion.button key={item.id} onClick={() => toggleChecklist(item.id)} whileTap={{ scale: 0.98 }}
                className={cn("w-full flex items-center gap-3 py-3 px-3 rounded-xl transition-all text-left",
                  checklist[item.id] ? "bg-blue-900/20" : "hover:bg-slate-800/60")}>
                <div className={cn("w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 transition-all",
                  checklist[item.id] ? "bg-blue-500 border-blue-500" : "border-slate-600")}>
                  {checklist[item.id] && <Check className="w-3.5 h-3.5 text-white" />}
                </div>
                <span className={cn("text-sm", checklist[item.id] ? "line-through text-slate-500" : "text-slate-200")}>{item.label}</span>
              </motion.button>
            ))}
          </div>
        </motion.div>
